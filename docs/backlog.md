...
3. ✅ **`waitForMany({ exact: true })` при переборе впустую ждёт весь таймаут** *(сделано: master `d578ffa`, порт на v1 `f14a083`; при получении `count + 1`-го сообщения промис немедленно реджектится с сообщением «exactly N time(s), but got M» вместо ожидания таймаута; тесты — `src/tests/events/waiter-many.spec.ts`).*
